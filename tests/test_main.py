...
def test_predict_big_file(m, tmpdir):
    ...
    df = m.predict_file(
        csv_file=csv_file, root_dir=os.path.dirname(csv_file), savedir=tmpdir
    )
...
def test_predict_tile(m):
    ...
    prediction = m.predict_tile(
        raster_path=raster_path, patch_size=300, patch_overlap=0.5, return_plot=False
    )
    assert isinstance(prediction, pd.DataFrame)
    assert set(prediction.columns) == {
        "xmin",
    ...
    assert set(soft_nms_pred.columns) == {
        "xmin",
        "ymin",
        "xmax",
        "ymax",
    ...
    prediction = m.predict_tile(
        image=image, patch_size=300, patch_overlap=0.5, return_plot=False
    )
    assert not prediction.empty

    # Test no non-max suppression
    prediction = m.predict_tile(
        raster_path=raster_path, patch_size=300, patch_overlap=0, return_plot=False
    )
    assert not prediction.empty
...
def test_evaluate_multiple_images(m, tmpdir):
    ...
    shutil.copyfile(
        "{}/OSBS_029.tif".format(original_root_dir), "{}/OSBS_029.tif".format(root_dir)
    )
    shutil.copyfile(
        "{}/OSBS_029.tif".format(original_root_dir),
