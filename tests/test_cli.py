...
PREDICT_SCRIPT = files("deepforest.scripts").joinpath("predict.py")
TRAIN_SCRIPT = files("deepforest.scripts").joinpath("train.py")
...
def test_predict_cli(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, PREDICT_SCRIPT] + args,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
    )
...
def test_predict_cli_config_help(tmp_path):
    ...
    result = subprocess.run(
        [sys.executable, PREDICT_SCRIPT, "--show-config"],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
    )
