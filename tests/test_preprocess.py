...
def test_compute_windows(config, image):
    windows = preprocess.compute_windows(
        image, config["patch_size"], config["patch_overlap"]
    )
...
def test_select_annotations(config, image):
    ...
    selected_annotations = preprocess.select_annotations(
        image_annotations, windows, index=7
    )
...
def test_select_annotations_tile(config, image):
    ...
    image_annotations = pd.read_csv("tests/data/OSBS_029.csv")
    selected_annotations = preprocess.select_annotations(
        image_annotations, windows, index=10
    )
...
def test_split_raster_empty_crops(config, tmpdir):
    ...
    # Returns a 6 column pandas array
    assert not annotations_file[
        (annotations_file.xmin == 0) & (annotations_file.xmax == 0)
    ].empty
