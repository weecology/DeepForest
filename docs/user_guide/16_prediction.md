...
```bash
> deepforest-predict --help

usage: deepforest-predict [-h] [-o OUTPUT] [--plot] [--show-config] [input]

DeepForest prediction utility
...
options:
  ...
  --plot                Plot results
  --show-config         Show available config overrides and exit
```
...
To see the default configuration and to check what options you can set, use the `--show-config` flag (no other options are required).
