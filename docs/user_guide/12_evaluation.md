...
We provide a command line tool to easily create evaluation results:

```bash
usage: deepforest evaluate [-h] [--root-dir ROOT_DIR] [--predictions PREDICTIONS] [--save-predictions SAVE_PREDICTIONS] [-o OUTPUT] [csv_file]

positional arguments:
  csv_file              Path to ground truth CSV file (optional if specified in config)

options:
  -h, --help            show this help message and exit
  --root-dir ROOT_DIR   Root directory containing images. Defaults to CSV directory if not specified.
  --predictions PREDICTIONS
                        Path to existing predictions CSV file. If not provided, predictions will be generated.
  --save-predictions SAVE_PREDICTIONS
                        Path to save generated predictions CSV (only used when --predictions is not provided)
  -o OUTPUT, --output OUTPUT
                        Path to save evaluation metrics summary CSV

Any remaining arguments <key>=<value> will be passed to Hydra to override the current config.
```

If you already have predictions generated by a model, then you can pass this as a CSV (use `deepforest predict`). If you don't, DeepForest will generate predictions before running the evaluation pass. You can save predictions as well, for example if you want to re-run evaluation with different settings. As with prediction, you need to tell DeepForest where to find your imagery (`--root_dir`); the tool will attempt to guess based on where the CSV file is, but it's best to provide a path. To store results for later, you can use the `-o/--output` flag.
