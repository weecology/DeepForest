...
class RecallPrecision(Metric):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.task == "box":
            ...
            # Use boxes to count instances and detect empty frames
            self.pred_key = "boxes"
    ...
    def update(
        ...
        Args:
            preds: List of prediction dicts (keys: ``boxes``, ``labels``,
                ``scores``; plus ``masks`` for the ``polygon`` task).
            targets: List of target dicts (keys: ``boxes``, ``labels``; plus
                ``masks`` for the ``polygon`` task).
