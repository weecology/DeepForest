...
class CropModel(LightningModule):
    ...
    def write_crops(self, root_dir, images, boxes, labels, savedir):
        """Write crops to disk.... Args: root_dir (str): The root directory
        where the images are located. images (list): A list of image filenames.
        boxes (list): A list of bounding box coordinates in the format [xmin,
        ymin, xmax, ymax]. labels (list): A list of labels corresponding to
        each bounding box. savedir (str): The directory where the cropped
        ...
        for index, box in enumerate(boxes):
            ...
            with rasterio.open(os.path.join(root_dir, image)) as src:
                ...
                square_box = self.expand_bbox_to_square([xmin, ymin, xmax, ymax], image_width, image_height)
