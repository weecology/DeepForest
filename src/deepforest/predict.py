...
def _predict_crop_model_(crop_model,
    ...
    if crop_model.numeric_to_label_dict is not None:
        results[f"cropmodel_label_{model_index}"] = [
            crop_model.numeric_to_label_dict[x] for x in label
        ]
    else:
        results[f"cropmodel_label_{model_index}"] = label

    results[f"cropmodel_score_{model_index}"] = score
