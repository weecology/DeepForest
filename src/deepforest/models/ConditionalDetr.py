...
class ConditionalDetrWrapper(nn.Module):
    """This class wraps a transformers ConditionalDetrForObjectDetection model
    so that input pre- and post-processing happens transparently."""

    def __init__(self, config, name, revision, use_nms=False, **hf_args):
        """Initialize a ConditionalDetrForObjectDetection model.
        ...
        with warnings.catch_warnings():
            ...
            self.net = ConditionalDetrForObjectDetection.from_pretrained(
                name,
                revision=revision,
...
class Model(BaseModel):
    ...
    def create_model(
        self,
        pretrained: str | Path | None = "microsoft/conditional-detr-resnet-50",
        *,
        revision: str | None = "main",
        map_location: str | torch.device | None = None,
        **hf_args,
    ) -> ConditionalDetrWrapper:
        """Create a Conditional DETR model from pretrained weights.
