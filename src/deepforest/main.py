...
from deepforest.metrics import BoxRecallPrecision
...
class deepforest(pl.LightningModule):
    ...
    def setup_metrics(self):
        ...
        self.box_recall_metric = BoxRecallPrecision(
        ...
        if len(targets) > 0:
            ...
            for i, target in enumerate(targets):
                ...
                if target["boxes"].shape[0] == 0:
                    has_predictions = len(preds[i]["boxes"]) > 0
                    # Update metric: ground truth is 0 (empty), prediction is 1 if model predicted boxes
                    self.empty_frame_accuracy.update(
                        torch.tensor([1.0 if has_predictions else 0.0]),
                        torch.tensor([0.0]),
                    )
                else:
            ...
            # Box recall metric can handle empty frames internally
            self.box_recall_metric.update(preds, image_names)
    ...
    def _compute_epoch_metrics(self) -> dict:
        ...
        metrics.update(self.box_recall_metric.compute())
