...
def load_coco_with_bboxes(json_file):
    """Load COCO format JSON file with bounding boxes (bbox) instead of
    segmentation.
    ...
    if "categories" in coco_data:
        category_ids = {
            cat["id"]: cat.get("name", f"category_{cat['id']}")
...
def process_izembek_with_splitting(df, root_dir, output_dir, image_files_map):
    ...
    for img_path in unique_images:
        ...
        temp_annotations_file = os.path.join(
            crops_dir, f"temp_{image_basename}_annotations.csv"
        )
        ...
        try:
            ...
            for crop_img_path in crop_df["image_path"].unique():
                ...
                crop_df.loc[crop_df["image_path"] == crop_img_path, "image_path"] = (
                    symlink_name
                )
...
def filter_small_boxes(df, min_area=1, epsilon=1e-6):
    ...
    if removed_count > 0:
        print(
            f"  Removed {removed_count} bounding boxes with area <= {min_area} pixel(s) or single-pixel dimensions"
        )
...
def main():
    ...
    for source_path in DATA_SOURCES:
        ...
        root_dir = (
            df.root_dir if hasattr(df, "root_dir") else os.path.dirname(source_path)
        )
        ...
        if "izembek" in source_path.lower():
            print(
                "  Using split_raster to create 2000-pixel crops with allow_empty=False"
            )
            df = process_izembek_with_splitting(
                df, root_dir, args.output_dir, image_files_map
            )
            if df.empty:
            ...
            print(
                f"  Loaded {len(df)} crop annotations from {df['image_path'].nunique()} crop images"
            )
            continue
    ...
    if not negative_coords_df.empty:
        ...
        if negative_after.empty:
            ...
            print(
                f"  Warning: {len(negative_after)} annotations still have negative coordinates after clipping"
            )
    else:
    ...
    if EXISTING_TEST and os.path.exists(EXISTING_TEST):
        ...
        print(
            f"Saved test annotations: {test_csv} ({len(test_df)} annotations, {len(test_images)} images)"
        )
