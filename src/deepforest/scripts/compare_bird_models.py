"""Compare retrained bird model checkpoint with pretrained
weecology/deepforest-bird model.
...
def compare_models(checkpoint_path, data_dir, iou_threshold=0.4):
    """Compare checkpoint model with pretrained weecology/deepforest-bird
    model.
    ...
    print(
        f"  Empty Frame Accuracy: {checkpoint_validate.get('empty_frame_accuracy', 'N/A')}"
    )
    ...
    if checkpoint_recall_evaluate is not None:
        ...
        print("  Box Recall: N/A")
    print(
        f"  Empty Frame Accuracy: {checkpoint_evaluate.get('empty_frame_accuracy', 'N/A')}"
    )
    ...
    if pretrained_recall_validate is not None:
        ...
        print("  Box Recall: N/A")
    print(
        f"  Empty Frame Accuracy: {pretrained_validate.get('empty_frame_accuracy', 'N/A')}"
    )
    ...
    if pretrained_recall_evaluate is not None:
        ...
        print("  Box Recall: N/A")
    print(
        f"  Empty Frame Accuracy: {pretrained_evaluate.get('empty_frame_accuracy', 'N/A')}"
    )
    ...
    if (
        checkpoint_precision_validate is not None
        and pretrained_precision_validate is not None
    ):
        ...
        if pretrained_precision_validate != 0:
            print(
                f"  Difference:  {precision_diff:+.4f} ({precision_diff / pretrained_precision_validate * 100:+.2f}%)"
            )
        else:
    ...
    if checkpoint_recall_validate is not None and pretrained_recall_validate is not None:
        ...
        if pretrained_recall_validate != 0:
            print(
                f"  Difference:  {recall_diff:+.4f} ({recall_diff / pretrained_recall_validate * 100:+.2f}%)"
            )
        else:
    ...
    if (
        "empty_frame_accuracy" in checkpoint_validate
        and "empty_frame_accuracy" in pretrained_validate
    ):
    ...
    if (
        checkpoint_precision_evaluate is not None
        and pretrained_precision_evaluate is not None
    ):
        ...
        if pretrained_precision_evaluate != 0:
            print(
                f"  Difference:  {precision_diff:+.4f} ({precision_diff / pretrained_precision_evaluate * 100:+.2f}%)"
            )
        else:
            print(f"  Difference:  {precision_diff:+.4f} (N/A%)")
    else:
    ...
    if checkpoint_recall_evaluate is not None and pretrained_recall_evaluate is not None:
        ...
        if pretrained_recall_evaluate != 0:
            print(
                f"  Difference:  {recall_diff:+.4f} ({recall_diff / pretrained_recall_evaluate * 100:+.2f}%)"
            )
        else:
            print(f"  Difference:  {recall_diff:+.4f} (N/A%)")
    else:
    ...
    if (
        "empty_frame_accuracy" in checkpoint_evaluate
        and "empty_frame_accuracy" in pretrained_evaluate
    ):
...
def evaluate_multiple_thresholds(
    ...
    for i, threshold in enumerate(thresholds):
        ...
        print(
            f"  trainer.validate() - Precision: {precision_validate:.4f}, Recall: {recall_validate:.4f}"
        )
        ...
        print(
            f"  main.evaluate() - Precision: {precision_evaluate:.4f}, Recall: {recall_evaluate:.4f}"
        )
    ...
    print("-" * 40)
    for thresh, prec, rec in zip(
        thresholds, precision_scores_validate, recall_scores_validate, strict=True
    ):
    ...
    print("-" * 40)
    for thresh, prec, rec in zip(
        thresholds, precision_scores_evaluate, recall_scores_evaluate, strict=True
    ):
        print(f"{thresh:<12.2f} {prec:<12.4f} {rec:<12.4f}")
    ...
    plt.plot(
        ...
        precision_scores_validate,
        "o-",
        label="Precision (trainer.validate())",
        linewidth=2,
        markersize=8,
        color="blue",
    )
    ...
    plt.title(
        "Precision and Recall vs Score Threshold\n(Retrained Bird Detection Model - Both Methods)",
        fontsize=14,
    )
