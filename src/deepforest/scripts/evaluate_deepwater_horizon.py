...
def load_shapefiles_and_create_test_csv(data_dir, output_csv="test.csv", output_dir=None):
    ...
    if os.path.exists(output_path):
        ...
        try:
            ...
            print(
                f"Found existing {output_csv} with {len(existing_df)} annotations from {len(existing_df['image_path'].unique())} images"
            )
...
def split_test_images_for_evaluation(
    test_csv,
    data_dir,
    patch_size=800,
    patch_overlap=0,
    ...
    """Split test images into smaller patches for evaluation using
    split_raster.
    ...
    for image_name in unique_images:
        ...
        temp_annotations_file = os.path.join(
            output_dir, f"temp_{image_name}_annotations.csv"
        )
        ...
        if not split_df.empty:
            ...
            print(
                f"  Created {len(split_df['image_path'].unique())} patches with {len(split_df)} annotations"
            )
        else:
    ...
    if all_split_annotations:
        ...
        raise ValueError(
            "No split annotations were created. Check that images exist and contain valid annotations."
        )
...
def generate_visualizations(
    ...
    for image_name in unique_images:
        ...
        checkpoint_predictions = checkpoint_model.predict_tile(
            path=image_path, patch_size=800, patch_overlap=0
        )

        # Predict with old model
        pretrained_predictions = pretrained_model.predict_tile(
            path=image_path, patch_size=800, patch_overlap=0
        )
        ...
        if len(checkpoint_predictions) > 0:
            ...
            plt.savefig(
                os.path.join(plots_dir, f"{base_name}_new.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.close(fig)
        ...
        if len(pretrained_predictions) > 0:
            ...
            plt.savefig(
                os.path.join(plots_dir, f"{base_name}_old.png"),
                dpi=300,
                bbox_inches="tight",
            )
            plt.close(fig)
        ...
        img1 = img1.resize(
            (int(img1.width * height / img1.height), height), PILImage.Resampling.LANCZOS
        )
        img2 = img2.resize(
            (int(img2.width * height / img2.height), height), PILImage.Resampling.LANCZOS
        )
...
def main():
    ...
    print(
        f"  Empty Frame Accuracy: {results['checkpoint'].get('empty_frame_accuracy', 'N/A')}"
    )
    ...
    print(
        f"  Empty Frame Accuracy: {results['pretrained'].get('empty_frame_accuracy', 'N/A')}"
    )
