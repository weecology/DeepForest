...
def run():
    ...
    print(
        f"  Empty Frame Accuracy: {zero_shot_metrics.get('empty_frame_accuracy', 'N/A')}"
    )

    # log the zero-shot evaluation results to the comet logger
    if comet_logger:
        comet_logger.experiment.log_metric(
            "zero_shot_box_precision", zero_shot_metrics.get("box_precision", "N/A")
        )
        comet_logger.experiment.log_metric(
            "zero_shot_box_recall", zero_shot_metrics.get("box_recall", "N/A")
        )
        comet_logger.experiment.log_metric(
            "zero_shot_empty_frame_accuracy",
            zero_shot_metrics.get("empty_frame_accuracy", "N/A"),
        )

    if comet_logger:
        # Log global steps
        global_steps = torch.tensor(
            m.trainer.global_step, dtype=torch.int32, device=m.device
        )
