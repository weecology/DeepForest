...
This script wraps evaluate_deepwater_horizon.py to evaluate multiple
patch sizes for both checkpoint and pretrained models, and generate a
sensitivity plot showing how metrics vary with patch size for
comparison.
"""
...
spec = importlib.util.spec_from_file_location(
    "evaluate_deepwater_horizon", _eval_module_path
)
...
def evaluate_patch_size_sensitivity(
    ...
    """Evaluate both checkpoint and pretrained models across multiple patch
    sizes and collect results.
    ...
    for patch_size in patch_sizes:
        ...
        print(
            f"  Checkpoint - Precision={checkpoint_precision}, Recall={checkpoint_recall}"
        )
        print(
            f"  Pretrained - Precision={pretrained_precision}, Recall={pretrained_recall}"
        )
...
def plot_sensitivity(results_df, output_path):
    """Create a plot showing sensitivity of metrics to patch size for both
    models.
    ...
    ax.set_title(
        "Sensitivity of Box Precision and Recall to Patch Size\n(Checkpoint vs Pretrained Model)",
        fontsize=14,
    )
