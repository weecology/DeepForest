...
class deepforest(pl.LightningModule):
    ...
    def __init__(self, num_classes=1, label_dict={"Tree": 0}, transforms=None, config_file='deepforest_config.yml'):
        ...
        # freezeing regression heads
        if self.config["freeze_regression_head"] is not None:

            if self.config["freeze_regression_head"]["tree"] * self.config["freeze_regression_head"]["bird"] == 0:
                if self.config["freeze_regression_head"]["tree"] > 0:
                    m = deepforest(
                        num_classes=self.config["freeze_regression_head"]["tree"], label_dict=self.label_dict)
                    deepforest_release_model = deepforest()
                    deepforest_release_model.use_release()
                    m.model.backbone.load_state_dict(
                        deepforest_release_model.model.backbone.state_dict())
                    m.model.head.regression_head.load_state_dict(
                        deepforest_release_model.model.head.regression_head.state_dict())

                else:
                    m = deepforest(
                        num_classes=self.config["freeze_regression_head"]["bird"], label_dict=self.label_dict)
                    deepforest_release_model = deepforest()
                ...
                print("Only one class type should be specified")
        ...
        if not len(label_dict) == num_classes:
            raise ValueError(
                'label_dict {} does not match requested number of classes {}, please supply a label_dict argument {{"label1":0, "label2":1, "label3":2 ... etc}} for each label in the dataset'.format(
                    label_dict, num_classes)
            )
    ...
    def predict_image(self, image=None, path=None, return_plot=False, color=None, thickness=1):
        ...
        if not type(image) == np.ndarray:
            raise TypeError(
                "Input image is of type {}, expected numpy, if reading from PIL, wrap in np.array(image).astype(float32)".format(type(image)))
        ...
        if not self.config["validation"]["csv_file"] == None:
            if (self.current_epoch + 1) % self.config["validation"]["val_accuracy_interval"] == 0:
                results = self.evaluate(
                    csv_file=self.config["validation"]["csv_file"], root_dir=self.config["validation"]["root_dir"])
                ...
                if not type(results["class_recall"]) == type(None):
                    for index, row in results["class_recall"].iterrows():
                        self.log("{}_Recall".format(
                            self.numeric_to_label_dict[row["label"]]), row["recall"])
                        self.log("{}_Precision".format(
                            self.numeric_to_label_dict[row["label"]]), row["precision"])
